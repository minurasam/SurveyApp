...
(function (scope) {
  ...
  function FastTextEncoder() {// This does not accept an encoding, and always uses UTF-8:
  ...
  function decodeSyncXHR(bytes) {
    ...
    var u = URL.createObjectURL(b); // This hack will fail in non-Edgium Edge because sync XHRs are disabled (and
  ...
  function decodeFallback(bytes) {
    var inputIndex = 0; // Create a working buffer for UTF-16 code points, but don't generate one
    ...
    for (;;) {
      var more = inputIndex < bytes.length; // If there's no more data or there'd be no room for two UTF-16 values,
      ...
      if ((byte1 & 0x80) === 0) {
        ...
        // 3-byte
        var _byte = bytes[inputIndex++] & 0x3f;

        var byte3 = bytes[inputIndex++] & 0x3f;
        pending[pendingIndex++] = (byte1 & 0x1f) << 12 | _byte << 6 | byte3;
      } else if ((byte1 & 0xf8) === 0xf0) {
        // 4-byte
        var _byte2 = bytes[inputIndex++] & 0x3f;

        var _byte3 = bytes[inputIndex++] & 0x3f;

        var byte4 = bytes[inputIndex++] & 0x3f; // this can be > 0xffff, so possibly generate surrogates
