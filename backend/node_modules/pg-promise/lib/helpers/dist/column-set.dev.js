...
var _require3 = require("./table-name"),
    TableName = _require3.TableName;

var _require4 = require("./column"),
...
function (_InnerState) {
  _inherits(ColumnSet, _InnerState);
  ...
  function ColumnSet(columns, opt) {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(ColumnSet).call(this));
    ...
    if (!npm.utils.isNull(opt.table)) {
      _this.table = opt.table instanceof TableName ? opt.table : new TableName(opt.table);
    }
    /**
    ...
    if (Array.isArray(columns)) {
      var colNames = {};
      _this.columns = columns.map(function (c) {
        var col = c instanceof Column ? c : new Column(c);

        if (col.name in colNames) {
          throw new Error("Duplicate column name \"".concat(col.name, "\"."));
        }
      ...
      if (columns instanceof Column) {
        ...
        for (var name in columns) {
          if (opt.inherit || Object.prototype.hasOwnProperty.call(columns, name)) {
            _this.columns.push(new Column(name));
          }
        }
      }
    }

    Object.freeze(_this.columns);
    ...
    for (var i = 0; i < _this.columns.length; i++) {
      var c = _this.columns[i]; // ColumnSet is simple when the source objects require no preparation,
      // and should be used directly:

      if (c.prop || c.init || "def" in c) {
        _this._inner.isSimple = false;
        break;
      }
    }
  ...
  _createClass(ColumnSet, [{
    ...
    get: function get() {
      ...
      if (!_i.names) {
        _i.names = this.columns.map(function (c) {
    ...
    get: function get() {
      ...
      if (!_i.variables) {
        _i.variables = this.columns.map(function (c) {
...
ColumnSet.prototype.assign = function (options) {
  ...
  var hasSource = options && options.source && typeof options.source === "object";
  ...
  list = list.map(function (c) {
    return prefix + c.escapedName + "=" + c.variable + c.castText;
  }).join();
...
ColumnSet.prototype.assignColumns = function (options) {
  ...
  var skip = typeof options.skip === "string" && [options.skip] || (Array.isArray(options.skip) || typeof options.skip === "function") && options.skip;
  var from = typeof options.from === "string" && options.from && npm.formatting.as.alias(options.from) + "." || "";
  var to = typeof options.to === "string" && options.to && npm.formatting.as.alias(options.to) + "." || "";
  var iterator = typeof skip === "function" ? function (c) {
    return !skip.call(c, c);
  } : function (c) {
    return skip.indexOf(c.name) === -1;
  };
  var cols = skip ? this.columns.filter(iterator) : this.columns;
  return cols.map(function (c) {
    return to + c.escapedName + "=" + from + c.escapedName;
  }).join();
};
/**
