...
var _require2 = require("./utils/color"),
...
function poolConnect(ctx, db, config) {
  return config.promise(function (resolve, reject) {
    ...
    if (p.ending) {
      db.$destroy();
      var err = new Error(npm.text.poolDestroyed);
      ...
      if (err) {
        ...
        resolve({
          ...
          release: function release() {
            client.end = end;
            var p = config.promise(function (res, rej) {
              return client.end().then(res)["catch"](rej);
            });
            Events.disconnect(ctx, client);
            client.removeListener("error", onError);
            return p;
...
function onError(err) {
  var ctx = this.$ctx;
  var cn = npm.utils.getSafeConnection(ctx.cn);
  Events.error(ctx.options, err, {
    cn: cn,
    dc: ctx.dc
  });
...
function lockClientEnd(client) {
  ...
  client.end = function (doNotCall) {
    ...
    ColorConsole.error("".concat(npm.text.clientEnd, "\n").concat(npm.utils.getLocalStack(1, 3), "\n"));
...
function setSchema(client, ctx) {
  ...
  if (typeof s === "string" || Array.isArray(s) && s.length) {
    client.query(npm.formatting.as.format("SET search_path TO $1:name", [s]), function (err) {
      // istanbul ignore if;
      if (err) {
...
module.exports = function (config) {
  return {
    pool: function pool(ctx, db) {
      return poolConnect(ctx, db, config);
    },
