...
var _require3 = require("./special-query"),
    resultQuery = _require3.resultQuery,
    multiResultQuery = _require3.multiResultQuery,
    streamQuery = _require3.streamQuery;

var _require4 = require("./context"),
    ConnectionContext = _require4.ConnectionContext;

var _require5 = require("./database-pool"),
    DatabasePool = _require5.DatabasePool;

var _require6 = require("./query-result"),
    queryResult = _require6.queryResult;
...
function Database(cn, dc, config) {
  ...
  pool.end = function (cb) {
    var res = endMethod.call(pool, cb);
    dbThis.$destroy();
    return res;
  };
  ...
  this.connect = function (options) {
    ...
    var self = {
      ...
      batch: function batch(values, opt) {
        return config.$npm.spex.batch.call(this, values, opt);
      },
      page: function page(source, opt) {
        return config.$npm.spex.page.call(this, source, opt);
      },
      sequence: function sequence(source, opt) {
        return config.$npm.spex.sequence.call(this, source, opt);
      }
    };
    var connection = options.direct ? config.$npm.connect.direct(ctx) : config.$npm.connect.pool(ctx, dbThis);
  ...
  this.query = function (query, values, qrm) {
    var self = this,
        ctx = createContext();
    return config.$npm.connect.pool(ctx, dbThis).then(function (db) {
      ctx.connect(db);
  ...
  npm.utils.addReadProp(this, "$destroy", function () {
    if (!destroyed) {
      if (!pool.ending) {
        endMethod.call(pool);
      }
  ...
  function transform(value, cb, thisArg) {
    return typeof cb === "function" ? value.then(function (data) {
  ...
  function extend(ctx, obj) {
    ...
    obj.one = function (query, values, cb, thisArg) {
      var v = obj.query.call(this, query, values, queryResult.one);
    ...
    obj.oneOrNone = function (query, values, cb, thisArg) {
      var v = obj.query.call(this, query, values, queryResult.one | queryResult.none);
      return transform(v, cb, thisArg);
    };
    /**
    ...
    obj.result = function (query, values, cb, thisArg) {
      var v = obj.query.call(this, query, values, resultQuery);
      return transform(v, cb, thisArg);
    };
    /**
    ...
    obj.multi = function (query, values) {
      return obj.query.call(this, query, values, multiResultQuery).then(function (data) {
    ...
    obj.task = function () {
      var args = npm.pubUtils.taskArgs(arguments);
    ...
    obj.txIf = function () {
      ...
      try {
        ...
        var reusable = args.options.reusable;
    ...
    function taskProcessor(params, isTX) {
      ...
      var taskCtx = ctx.clone(); // task context object;
      ...
      var tsk = new config.$npm.task.Task(taskCtx, params.options.tag, isTX, config);
      ...
      return config.$npm.connect.pool(taskCtx, dbThis).then(function (db) {
        taskCtx.connect(db);
