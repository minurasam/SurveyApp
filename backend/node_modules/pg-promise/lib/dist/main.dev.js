...
var _require2 = require("./database-pool"),
    DatabasePool = _require2.DatabasePool;

var _require3 = require("./types"),
    PreparedStatement = _require3.PreparedStatement,
    ParameterizedQuery = _require3.ParameterizedQuery;

var _require4 = require("./query-file"),
    QueryFile = _require4.QueryFile;

var _require5 = require("./query-result"),
    queryResult = _require5.queryResult;

var _require6 = require("./promise-parser"),
    parsePromise = _require6.parsePromise;

var _require7 = require("assert-options"),
    assertOptions = _require7.assertOptions;
...
function $main(options) {
  ...
  var pg = npm.pg;
  var p = parsePromise(options.promiseLib);
  var config = {
  ...
  if (options.pgNative) {
    ...
    if (!originalClientConnect) {
      ...
      pg.Client.prototype.connect = function () {
        var _this = this,
            _originalClientConnec;

        var handler = function handler(msg) {
          if (msg.parameterName === "server_version") {
            _this.serverVersion = msg.parameterValue;

            _this.connection.removeListener("parameterStatus", handler);
          }
        };

        this.connection.on("parameterStatus", handler);
        return (_originalClientConnec = originalClientConnect).call.apply(_originalClientConnec, [this].concat(Array.prototype.slice.call(arguments)));
      };
    }
  }
