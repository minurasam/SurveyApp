 ...
 */
var _require = require("assert-options"),
    assertOptions = _require.assertOptions;
...
var maxVariable = 100000; // maximum supported variable is '$100000'
////////////////////////////////////////////////////
...
function formatValue(_ref) {
  ...
  var ctf = getCTF(value); // Custom Type Formatting
  ...
  switch (_typeof(value)) {
    ...
    case "symbol":
      throw new TypeError("Type Symbol has no meaning for PostgreSQL: ".concat(value.toString()));
...
function formatArray(array, options) {
  var loop = function loop(a) {
    return "[" + a.map(function (value) {
      return value instanceof Array ? loop(value) : formatValue({
        value: value,
        options: options
      });
    }).join() + "]";
  ...
  var prefix = options && options.capSQL ? "ARRAY" : "array";
...
var formatAs = {
  object: function object(_ref2) {
    ...
    return query.replace(npm.patterns.namedParameters, function (name) {
      var v = formatAs.stripName(name.replace(/^\$[{(<[/]|[\s})>\]/]/g, ""), raw),
          c = npm.utils.getIfHas(obj, v.name);

      if (!c.valid) {
        throw new Error("Invalid property name '".concat(v.name, "'."));
      }
      ...
      throw new Error("Property '".concat(v.name, "' doesn't exist."));
    });
  },
  array: function array(_ref3) {
    var query = _ref3.query,
        _array = _ref3.array,
        raw = _ref3.raw,
        options = _ref3.options;
    options = options && typeof options === "object" ? options : {};
    return query.replace(npm.patterns.multipleValues, function (name) {
      var v = formatAs.stripName(name.substr(1), raw);
      var idx = v.name - 1;

      if (idx >= maxVariable) {
        throw new RangeError("Variable $".concat(v.name, " exceeds supported maximum of $").concat(maxVariable));
      }
      ...
      if ("def" in options) {
        var d = options.def,
            value = typeof d === "function" ? d.call(_array, idx, _array) : d;
        return formatValue({
          value: value,
          fm: v.fm,
          options: options
      ...
      throw new RangeError("Variable $".concat(v.name, " out of range. Parameters array length: ").concat(_array.length));
    });
  },
  value: function value(_ref4) {
    ...
    return query.replace(npm.patterns.singleValue, function (name) {
...
function getCTF(value) {
  if (!isNull(value)) {
    var toPostgres = value[ctfSymbols.toPostgres],
...
function formatEntity(entity, values, _ref5) {
  var capSQL = _ref5.capSQL,
      type = _ref5.type;
  var prefix = type === "func" ? "select * from" : "call";

  if (capSQL) {
    prefix = prefix.toUpperCase();
  }

  return "".concat(prefix, " ").concat($as.alias(entity), "(").concat(formatCSV(values, {
    capSQL: capSQL
  }), ")");
}
...
var $as = {
  ...
  name: function name(_name) {
    ...
    if (_name) {
      if (typeof _name === "string") {
        return /^\s*\*(\s*)$/.test(_name) ? _name : formatSqlName(_name);
      }

      if (typeof _name === "object") {
        var keys = Array.isArray(_name) ? _name : Object.keys(_name);
        ...
        return keys.map(function (value) {
          if (!value || typeof value !== "string") {
            throw new Error("Invalid sql name: ".concat(npm.utils.toJson(value)));
          }
    ...
    throw new TypeError("Invalid sql name: ".concat(npm.utils.toJson(_name)));
  },
  ...
  alias: function alias(name) {
    ...
    if (name && typeof name === "string") {
      return name.split(".").filter(function (f) {
        return f;
      }).map(function (a) {
        var m = a.match(/^([a-z_][a-z0-9_$]*|[A-Z_][A-Z0-9_$]*)$/);
    ...
    throw new TypeError("Invalid sql alias: ".concat(npm.utils.toJson(name)));
  },
  ...
  buffer: function buffer(obj, raw) {
    ...
    throw new TypeError("".concat(wrapText(obj), " is not a Buffer object."));
  },
  ...
  date: function date(d, raw) {
    ...
    throw new TypeError("".concat(wrapText(d), " is not a Date object."));
  },
  ...
  number: function number(num) {
    ...
    if (t !== "number" && t !== "bigint") {
      throw new TypeError("".concat(wrapText(num), " is not a number."));
    }
   ...
   * @returns {string}
   */
  array: function array(arr, options) {
    ...
    throw new TypeError("".concat(wrapText(arr), " is not an Array object."));
  },
   ...
   * @returns {string}
   */
  func: function func(_func, raw, cc) {
    if (isNull(_func)) {
      throwIfRaw(raw);
      return "null";
    }
    ...
    if (typeof _func !== "function") {
      throw new TypeError("".concat(wrapText(_func), " is not a function."));
    }

    var fm = raw ? fmFlags.raw : null;
    return formatValue({
      value: resolveFunc(_func, cc),
      fm: fm,
      cc: cc
    });
  },
...
var $to = {
  ...
  csv: function csv(values, options) {
  ...
  buffer: function buffer(obj, raw) {
    var s = "\\x".concat(obj.toString("hex"));
    return raw ? s : wrapText(s);
  },
  date: function date(d, raw) {
    var s = npm.pgUtils.prepareValue(d);
    return raw ? s : wrapText(s);
  },
  json: function json(data, raw) {
    var s = npm.utils.toJson(data);
