...
var callback = function callback(ctx, obj, cb, config) {
  var $p = config.promise;
  var result;
...
var execute = function execute(ctx, obj, isTX, config) {
  var $p = config.promise; // updates the task context and notifies the client;
  ...
  var cbData, cbReason, success, spName; // Save-Point Name;

  var capSQL = ctx.options.capSQL; // capitalize sql;

  update(true);

  if (isTX) {
    // executing a transaction;
    spName = "sp_".concat(ctx.txLevel, "_").concat(ctx.nextTxCount);
    return begin().then(function () {
      return callback(ctx, obj, ctx.cb, config).then(function (data) {
...
module.exports = function (config) {
  var npmLocal = config.$npm; // istanbul ignore next:
