...
var _require2 = require("./query-file"),
    QueryFile = _require2.QueryFile;

var _require3 = require("./types"),
    ServerFormatting = _require3.ServerFormatting,
    PreparedStatement = _require3.PreparedStatement,
    ParameterizedQuery = _require3.ParameterizedQuery;

var _require4 = require("./special-query"),
    SpecialQuery = _require4.SpecialQuery;
...
var badMask = queryResult.one | queryResult.many; // unsupported combination bit-mask;
//////////////////////////////
// Generic query method;

function $query(ctx, query, values, qrm, config) {
  var special = qrm instanceof SpecialQuery && qrm;
  ...
  if (!error) {
    if (!pgFormatting && !npm.utils.isText(query)) {
      var errTxt = entityType ? entityType === "func" ? npm.text.invalidFunction : npm.text.invalidProc : npm.text.invalidQuery;
  ...
  if (!error && (!pgFormatting || entityType)) {
    try {
      ...
      if (entityType) {
        var prefix = entityType === "func" ? "select * from" : "call";
